...
function writeError(e) {
    throw { message: "[ERROR] " + e.message + " (line:" + e.line + ", column:" + e.column + ")" };
}
...
function loadStyleSheet(sheet, callback, reload, remaining) {
    //name.slice(0, name.lastIndexOf('/') + 1) + sheet.href;
    var sheetName = curPath + '/' + sheet.href;
    var input = readFile(sheetName);
    var parser = new less.Parser({
        // the paths option below was added in 1.3.0 dist of less-css
        // it breaks our usage but not sure why so we are documenting it here
        // for now. look into this.
...
function compile(scriptName, code, min) {
  if(!scriptName) throw { message: "empty script name " + scriptName };
